  ...
  fetch(`/api/get_home_events?${params}`)
      ...
      .then((result) => {
        ...
        if (data.value.length < 3) {
          ...
          for (let i = data.value.length; i < 3; i++) {
...
<template>
  <section class="fixed-top scrollable-content">
    ...
    <div class="container-fluid" v-if="loading===false">
      <div id="carouselExampleIndicators" class="carousel slide mx-4 mt-4" style="width: 90rem">
        ...
        <div class="carousel-inner" v-for="(item, index) in data" :key="item">
          <div class="carousel-item active" v-if="index===0">
            <img v-bind:src="item.Image" class="d-block w-100" alt="can not display image">
          </div>
      ...
      <ul class="nav nav-tabs mt-4 mx-4">
        <div class="row">
          <li class="nav-item">
            <a class="nav-link active" aria-current="page">Recent</a>
          </li>
        </div>
        <div class="row">
          <div class="card-group mb-4">
            <div class="card" v-for="item in data" :key="item" style="width: 90rem;">
              <img v-bind:src="item.Image" class="card-img-top HomeImage2" alt="can not display image">
              <div class="card-body HomeCard">
                <h5 class="card-title">{{ item.Title }}</h5>
                <p class="card-text">{{ item.Description }}</p>
                <p class="card-text"><small class="text-body-secondary"> {{
                    getTimeDifference(item.modifiedAt)
                  }} </small></p>
              </div>
            </div>
          </div>
